...
from crypto.transactions.builder.validator_resignation import ValidatorResignation

set_network(Devnet)


def test_validator_resignation_transaction(passphrase):
    """Test if validator resignation transaction gets built
    """
    transaction = ValidatorResignation()
...
def test_validator_resignation_transaction_custom_fee(passphrase):
    """Test if validator resignation transaction gets built with a custom fee
    """
    transaction = ValidatorResignation(5)
    transaction.set_nonce(1)
    transaction.set_type_group(TRANSACTION_TYPE_GROUP.CORE)
    transaction.sign(passphrase)
    transaction_dict = transaction.to_dict()
